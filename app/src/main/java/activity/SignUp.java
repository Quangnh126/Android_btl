...
public class SignUp extends AppCompatActivity {
        ...
        signup.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (Username.length() > 0)
                {
                    ...
                    String link = Sever.checkuser+"?username="+Username;
                    //Log.d("link",link);
                    //Log.d("s",link);
                    StringRequest request = new StringRequest(Request.Method.POST, link, new Response.Listener<String>() {
                        ...
                        public void onResponse(String response) {
                            ...
                            {
                                CheckInternetConnection.ShowToast_Short(getApplicationContext(),"username da ton tai");
                                Log.d("exist","true");
                                isexist=true;
                                return;
                            }
    ...
    private void SignUp(){
        ...
        final String Username = username.getText().toString().trim();
        final String Phone = phonenumber.getText().toString().trim();
        if (Username.length() > 0 && Repassword.length() > 0 && Repassword.equals(Password)) {
            Log.d("in", "in");
            RequestQueue queue = Volley.newRequestQueue(getApplicationContext());
